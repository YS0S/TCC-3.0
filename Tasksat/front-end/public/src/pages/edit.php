...
<div class="w-4/5 h-fit bg-white rounded-lg p-6 grid self-center justify-self-center gap-6 m-4">
    ...
    <form method="POST" action="/Tasksat/back-end/actions/edit.action.php" enctype="multipart/form-data" id="chamado-form">
        ...
        <div class="mt-3">
            <div class="">
                ...
                <?php if (!empty($chamado['foto'])): ?>
                    <img src="<?= '/Tasksat/front-end/storage/upload/' . rawurlencode($chamado['foto']) ?>" alt="Foto do chamado" width="250" style="margin-bottom:10px; border:1px solid #ddd; border-radius:4px;">
                <?php else: ?>
                    <p class="font-bold text-verde-250 text-lg">Nenhuma foto enviada.</p>
                <?php endif; ?>
                <br><label for="foto" class="font-bold text-verde-250 text-lg">Atualizar foto:</label>
        ...
        <?php if (count($historico) > 0): ?>
            <?php foreach ($historico as $h): ?>
                <div class="historico-item">
                    <p><span class="font-bold text-lg">Campo:</span> <?= esc($h['campo_alterado']) ?></p>
                    <p><span class="font-bold text-lg">Valor antigo:</span> <?= esc($h['valor_antigo']) ?></p>
                    <p><span class="font-bold text-lg">Valor novo:</span> <?= esc($h['valor_novo']) ?></p>
                    <p><span class="font-bold text-lg">Alterado por:</span> <?= esc($h['alterado_por']) ?></p>
                    <p><span class="font-bold text-lg">Data:</span> <?= esc($h['data_alteracao']) ?></p>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
...
<script>
    document.getElementById('chamado-form').addEventListener('submit', function(e) {
        // O envio real é feito para edit.action.php
    });
</script>
</body>
</html>